...
//!
//! Wire: unprefixed Base58Check(`version ‖ kind ‖ payload`) with a `SHA256d`
//! checksum — pure ASCII, so a ticket survives a URL or a shell word verbatim.
//! Every token in the crate shares this framing, so nothing is told apart by
//! sight: the `kind` byte marks this as a *blob* ticket and makes a wrong-kind
//! token (a bridge ticket) or a mesh id fail cleanly on decode. Mirrors the
//! application bridge ticket.
...
/// bridge ticket's kind so a token of the wrong kind is rejected on decode —
/// the only thing that tells the two apart now that neither is branded.
const KIND: u8 = 1;
...
impl BlobTicket {
    ...
    ///
    /// # Errors
    /// Invalid Base58, the wrong ticket kind, a bad checksum/version, or a
    /// malformed payload.
...
mod tests {
    ...
    #[test]
    fn encodes_to_bare_ascii_base58() {
        // The whole point of dropping the glyph: a ticket needs no
        // percent-encoding in a URL and no quoting in a shell word.
    ...
    fn rejects_a_mesh_token() {
        // Nothing distinguishes an id from a ticket by sight any more, so this
        // pins the thing that actually does the work: a mesh id's payload has
        // its own framing, so it fails the checksum or the kind byte.
        // (`kind == 1` here; a mesh id's second byte is its seed, not a kind.)
