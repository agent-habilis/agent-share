...
pub mod doc {
    ...
    /// go through [`change_body`].
    #[cfg(any(test, feature = "adversarial"))]
    pub use crate::doc::wire::merge_body;
    pub use crate::doc::wire::{change_body, encrypt_body};
}

/// Content-addressed payloads too large for a gossip frame.
pub mod blob {
    pub use crate::blob::{BlobTicket, ContentId, HASH_LEN, OffloadRequest, SECRET_LEN, fetch};
    // Serving and offloading read files from disk; fetching does not.
    #[cfg(feature = "host")]
    pub use crate::blob::{BlobServer, offload};
}
