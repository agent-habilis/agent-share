...
//! checksum. The framing is shared with the blob and application-bridge tickets
//! and with the mesh id; the `kind` byte is what tells them apart, so a token
//! of the wrong kind fails decode on the payload rather than on sight.
//!
...
impl InviteTicket {
    ...
    /// Decode an invite ticket — structural parse only.
    ///
    /// # Errors
    /// Invalid Base58, the wrong kind, a bad checksum/version, or a malformed
    /// payload.
    ...
    pub(crate) fn redeem(&self, password: Option<&Password>) -> Result<Mesh> {
...
mod tests {
    ...
    fn rejects_an_invite_signed_by_a_non_issuer_key() {
        ...
        let expiry = crate::util::clock::unix_secs() + 3600;
    ...
    fn rejects_an_expired_invite_even_with_a_valid_signature() {
        ...
        let hash = mesh.to_string();
        let root = *mesh.invite_key().unwrap();
        let past = crate::util::clock::unix_secs() - 100;
    ...
    fn a_mesh_id_is_not_an_invite() {
        // An id and an invite are both bare Base58; the kind byte and the
        // checksum are the only things keeping them apart.
        let plain = Mesh::new(
            [3u8; 32],
            MeshName::new("t").unwrap(),
            MeshConfig::loopback(),
    ...
    fn an_invite_and_a_blob_ticket_never_cross_parse() {
        ...
        // Same framing, different kind byte — each rejects the other.
