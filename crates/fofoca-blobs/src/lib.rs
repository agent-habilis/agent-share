...
//!
//! It also does not know what an `agent-share` is. It takes a key, a size, an
//! mtime and byte ranges. See `tests/isolation.rs`.
//!
//! # Both runtimes
//!
...
///
/// Measured rather than assumed (RFC 03, S0.3). Against 16 `KiB` this is **four
...
/// through generics rather than `dyn`, matching how `agent-share`'s own
/// `ByteSource` seam is consumed.
pub trait BlobStore {
