...
It also does not know what an `agent-share` is, and `tests/isolation.rs` fails
if it learns.
...
`MemStore` today. `FsStore` and `OpfsStore` follow, and each re-runs
`tests/conformance.rs` unchanged — that suite is the reason `BlobStore` is a
trait at all. The backends differ in every way except the behaviour it asserts,
and the expensive failures are all behaviour.
...
implementation and SIMD is the only lever; measured at ~1.8×. The feature is
*assumed* rather than detected, so enabling it without the flag produces a
module that fails validation.

## Design notes

`docs/rfc/03-fofoca-blobs/` in this repo, including the measurements the chunk
group size and the SIMD numbers come from.
