---
title: WebMCP
description: Drive the webapp with an agent through WebMCP tools.
---

# Driving the webapp with an agent
...
Registration lives in [`packages/agent-share-web/src/lib/webmcp/`](https://github.com/agent-habilis/agent-share/tree/main/packages/agent-share-web/src/lib/webmcp). It is feature-detected, so on a
...
```
navigate_page       https://agent-share.dev/app/files/<ticket>
...
`agent-share seed`. To pull bytes into the browser instead — where later reads
...
large tree is still `agent-share seed`'s job.
...
publish { files: [{ path: "notes.md", content: "# hi\n" }] }
  -> { ticket: "SvL6pH…", url: "https://…/app/files/SvL6pH…", files: 1 }

agent-share seed SvL6pH… ./out       # works from the CLI
```
