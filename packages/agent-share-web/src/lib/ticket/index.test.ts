...
// `parseRoute`, `previewSegments` and `shareUrl` all default to reading
// `window.location`. `scripts/test-setup.ts` resets it before every test, so a
// test that sets the URL cannot leak into the next one.
