...
import { respond, type StreamEntry } from '../lib/stream/range.ts'
import { STREAM_PREFIX, type PageMessage, type ReadReply } from './protocol.ts'
 ...
 * the same names with different types, and the whole `web/` workspace typechecks
 * as one project against `dom`. Adding a second tsconfig for one file costs more
